CREATE MATERIALIZED VIEW "user_materialized" AS SELECT "id", "email", "password", "last_name", "first_name", "gender", "city", "zip_code", "country" FROM "user" FRESHNESS MANUAL
CREATE MATERIALIZED VIEW "bid_materialized" AS SELECT "id" , "amount", "user", "auction" FROM "bid" FRESHNESS MANUAL
CREATE MATERIALIZED VIEW "picture_materialized" AS SELECT * FROM "picture" FRESHNESS MANUAL
CREATE MATERIALIZED VIEW "auction_materialized" AS SELECT "id", "title", "description", "category", "user" FROM "auction" FRESHNESS MANUAL
...
CREATE MATERIALIZED VIEW "auctionCategory_materialized" AS SELECT "id", "title", "category" FROM "auction" FRESHNESS MANUAL
